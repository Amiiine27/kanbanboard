...
function App() {
  ...
  const moveCard = (cardIndex, startColumn, endColumn) => {
    ...
    const newStartColumnCards = kanbanData[startColumn].filter((card, index) => index!==cardIndex);
  ...
  useEffect(() => {
    moveCard(0, "todo", "inProgress");
  }, []); // Le [] = "exécute une seule fois"
    ...
    <>
      <div className="min-h-screen max-h-screen bg-gray-100 p-4">
        ...
        <div className="flex gap-4 max-w-7xl mx-auto">
          <Column title="To Do" cardsTab={kanbanData.todo} />
          <Column title="In Progress" cardsTab={kanbanData.inProgress} />
          <Column title="Done" cardsTab={kanbanData.done} />
        </div>
      </div>
    </>
  );
